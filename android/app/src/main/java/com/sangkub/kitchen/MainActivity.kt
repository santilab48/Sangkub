...
class Api(private val ctx:Context){
 ...
 var token:String? get()=prefs.getString("access",null);private set
 fun logout(){prefs.edit().clear().apply();token=null}
 private fun req(url:String,b:Request.Builder=Request.Builder()):Request.Builder=b.url(BASE+url).header("apikey",KEY).apply{token?.let{header("Authorization","Bearer $it")}}
 suspend fun login(email:String,password:String):Result<Unit>=withContext(Dispatchers.IO){runCatching{val body=buildJsonObject{put("email",email);put("password",password)}.toString().toRequestBody(JSON);http.newCall(req("/auth/v1/token?grant_type=password").post(body).build()).execute().use{r->val s=r.body?.string().orEmpty();if(!r.isSuccessful)error(Json.parseToJsonElement(s).jsonObject["msg"]?.jsonPrimitive?.content?:"เข้าสู่ระบบไม่สำเร็จ");token=Json.parseToJsonElement(s).jsonObject["access_token"]!!.jsonPrimitive.content;prefs.edit().putString("access",token).apply()}}}
 suspend fun orders():List<KitchenOrder>=withContext(Dispatchers.IO){val url="/rest/v1/orders?select=id,order_no,status,tables(name),order_items(item_name,quantity,station)&status=in.(new,accepted,preparing,ready)&order=created_at.asc&limit=100";http.newCall(req(url).get().build()).execute().use{r->if(r.code==401){logout();error("SESSION")};if(!r.isSuccessful)error(r.body?.string().orEmpty());Json.parseToJsonElement(r.body!!.string()).jsonArray.map{o->val x=o.jsonObject;val table=x["tables"]?.jsonObject?.get("name")?.jsonPrimitive?.content?:"โต๊ะ";val foods=x["order_items"]?.jsonArray?.filter{it.jsonObject["station"]?.jsonPrimitive?.content=="kitchen"}?.map{"${it.jsonObject["quantity"]!!.jsonPrimitive.int}× ${it.jsonObject["item_name"]!!.jsonPrimitive.content}"}?:emptyList();KitchenOrder(x["id"]!!.jsonPrimitive.content,x["order_no"]!!.jsonPrimitive.long,table,x["status"]!!.jsonPrimitive.content,foods)}}}
 suspend fun status(id:String,status:String)=withContext(Dispatchers.IO){val body=buildJsonObject{put("p_order_id",id);put("p_status",status)}.toString().toRequestBody(JSON);http.newCall(req("/rest/v1/rpc/set_order_status").post(body).build()).execute().use{if(!it.isSuccessful)error(it.body?.string().orEmpty())}}
}
...
class MainActivity:ComponentActivity(),TextToSpeech.OnInitListener{
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);tts=TextToSpeech(this,this);if(Build.VERSION.SDK_INT>=33)notify.launch(Manifest.permission.POST_NOTIFICATIONS);setContent{MaterialTheme{KitchenApp(Api(this)){speak(it)}}}}
 override fun onInit(s:Int){if(s==TextToSpeech.SUCCESS){tts.language=Locale("th","TH");tts.setSpeechRate(.92f)}}
 private fun speak(s:String){tts.speak(s,TextToSpeech.QUEUE_ADD,null,System.nanoTime().toString())}
