...
class MainActivity:ComponentActivity(),TextToSpeech.OnInitListener{
 ...
 override fun onInit(status:Int){if(status==TextToSpeech.SUCCESS){val engine=tts?:return;val result=engine.setLanguage(Locale("th","TH"));ttsReady=result!=TextToSpeech.LANG_MISSING_DATA&&result!=TextToSpeech.LANG_NOT_SUPPORTED;engine.setSpeechRate(.92f);engine.setPitch(1f)}}
 private fun speak(text:String){if(!ttsReady){initTts();android.os.Handler(mainLooper).postDelayed({tts?.speak(text,TextToSpeech.QUEUE_ADD,null,"sangkub-${System.nanoTime()}")},1000)}else tts?.speak(text,TextToSpeech.QUEUE_ADD,null,"sangkub-${System.nanoTime()}")}
 override fun onResume(){super.onResume();if(tts==null)initTts()}
