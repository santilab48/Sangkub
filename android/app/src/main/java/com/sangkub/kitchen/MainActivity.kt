...
class Api {
    ...
    suspend fun orders(): List<KitchenOrder> = withContext(Dispatchers.IO) {
        ...
        http.newCall(req("/rest/v1/rpc/get_kitchen_orders").post(body).build()).execute().use { r ->
            ...
            if (!r.isSuccessful) error("เชื่อมต่อครัวไม่ได้ (${r.code})")
            Json.parseToJsonElement(text).jsonArray.map { e ->
                val x = e.jsonObject
                val foods = x["foods"]?.jsonArray?.map { f ->
                    "${f.jsonObject["quantity"]!!.jsonPrimitive.int} ${f.jsonObject["name"]!!.jsonPrimitive.content}"
                } ?: emptyList()
                KitchenOrder(x["id"]!!.jsonPrimitive.content, x["order_no"]!!.jsonPrimitive.long, x["table"]!!.jsonPrimitive.content, x["status"]!!.jsonPrimitive.content, foods)
            }
        }
    }

    suspend fun status(id: String, status: String) = withContext(Dispatchers.IO) {
        val body = "{\"p_device_token\":\"$DEVICE\",\"p_order_id\":\"$id\",\"p_status\":\"$status\"}".toRequestBody(MEDIA)
        http.newCall(req("/rest/v1/rpc/kitchen_set_order_status").post(body).build()).execute().use { r ->
            if (!r.isSuccessful) error("บันทึกไม่ได้ (${r.code})")
        }
    }
}
...
class MainActivity : ComponentActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent { MaterialTheme { KitchenApp(Api(), ttsStatus) { speak(it) } } }
    }
